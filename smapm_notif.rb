...
def mainmenu
 puts "select an option
   0: enter CDM
 Search Events in SAM
   10: search active events in SAM
 Search in Configuration files
   1: list/check NOTIF config file
   2: list events in NOTIF config file
   3: print event details from the running.ncf file for #{$cdm}
   4: search for calculated values for a given event in all CDMs
 Search in Log files
   20: list files with string in NOTIF logs for #{$cdm}
   21: search string in NOTIF log file
 EXIT Program
   9: Exit"
  case gets.strip
  when "0"
  ...
  when "10"
   Didatacommon.sam_search_events
  when "1"
   Didatacommon.notif_get_running_ncf_files
  when "2"
   Didatacommon.notif_list_events
  when "3"
   Didatacommon.notif_get_event_from_running_ncf
  when "4"
   Didatacommon.notif_search_calcvalues_all_cdms
  when "20"
   Didatacommon.notif_get_notif_data_list
  when "21"
   Didatacommon.notif_get_notif_data
  when "9"
    $menu=1
    puts "exiting..."
    exit
