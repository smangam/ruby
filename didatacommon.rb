...
##################################
# methods to get information from the user
##################################
...
def self.fs_opt_cleanup
  cmd = "ssh root@"+$cdm +" df -m|grep opt | grep -v Filesystem|awk '{print $5,$4/1024,$6}'|sed 's/%//g' | awk '{if ($1 > 89) print $1,$2,$3}'"
  output = `#{cmd}`.chomp
  puts "DISK USAGE for #{$cdm} [% MB filesystem] " + output

  # check if APG qdf data is filling up. if so, exit from this method
  cdm_type($cdm) 
  if $cdm_type !="base" 
    cdm_std_dirs($cdm)
    cmd = "ssh root@"+$cdm +" ls -ltr #{$apg_cdm_failover_storage_dir} |wc -l 2>/dev/null"
    output = `#{cmd}`.chomp
    if (output.to_i > 3) then
      puts "DISK FULL for #{$cdm} due to APG qdf files filling up. Total files: " + output
    end
  else
    $cdm_tenant_list.each { |x|
      cdm_type(x)
      cdm_std_dirs(x)
      cmd = "ssh root@"+x +" ls -ltr #{$apg_cdm_failover_storage_dir} |wc -l 2>/dev/null"
      puts cmd
      output = `#{cmd}`.chomp
      if (output.to_i > 3) then
        puts "DISK FULL for #{x} due to APG qdf files filling up. Total files: " + output
      end
    }
  end
  ...
  if $cdm_type != "base"
    ...
    $cdm_tenant_list.each { |x|
      cdm_type(x)
      cdm_std_dirs(x)
      cmd = "ssh root@"+x +" find #{$samdir} -mtime +#{$samlog_days_keep} -size +1M 2>/dev/null"
      puts cmd
      output = `#{cmd}`
      if ($?.success?) then
        puts "deleting samlog files older than #{$samlog_days_keep} days for #{x} "
        output_arr = output.split(/\n/)
        output_arr.each { |f|
          cmd = "ssh root@"+x +" rm -rf #{f}"
          `#{cmd}`
        }
      end
    }
  ...
  if $cdm_type != "base" 
    ...
    $cdm_tenant_list.each { |x|
      cdm_type(x)
      cdm_std_dirs(x)
      cmd = "ssh root@"+x +" find #{$samdir}/archives -mtime +10 -size +1M 2>/dev/null"
      puts cmd
      output = `#{cmd}`
      if ($?.success?) then
        puts "deleting samlog archive files older than 10 days for #{x} "
        output_arr = output.split(/\n/)
        output_arr.each { |f|
          cmd = "ssh root@"+x +" rm -rf #{f}"
          `#{cmd}`
  ...
  if $cdm_type != "base"
    ...
    $cdm_tenant_list.each { |x|
      cdm_type(x)
      cdm_std_dirs(x)
      cmd = "ssh root@"+x +" find #{$samdir} -mtime +7 -size +1M|grep -v '.gz'"
      output = `#{cmd}`
      if ($?.success?) then
        puts "zipping the file for #{$cdm} "
        output_arr = output.split(/\n/)
        output_arr.each { |f|
          cmd = "ssh root@"+x +" gzip #{f}"
          `#{cmd}`
        }  
      end
    }
  ...
  if (output.to_i > 4) then
    ...
    if ($cdm_type == "solo") then
      ...
      cmd = "ssh root@"+$cdm +" rm -rf /opt/InCharge9/SAM/smarts/customer/#{$cdm}/logs/archives/*audit*"
    end
    `#{cmd}`
  end
  ...
  # delete catalina.out if larger than 5GB
  ...
  output = `#{cmd}`
  if ($?.success?) then
    puts "size of /opt/smarts-ncm/ncmcore/java_pid*hprof is >5GB. deleting... for #{$cdm} "
    output_arr = output.split(/\n/)
    output_arr.each do |f|
      cmd = "ssh root@"+$cdm +" rm -rf #{f}"
      puts cmd
  ...
  output = `#{cmd}`
  if ($?.success?) then
    puts "size of /opt/smarts-ncm/ncmcore/logs is >5GB. deleting... for #{$cdm} "
    output_arr = output.split(/\n/)
    output_arr.each do |f|
      cmd = "ssh root@"+$cdm +" rm -rf #{f}"
      puts cmd
...
def self.pending_jobs_AS_all
  f=File.new("/root/smangam/#{$ncm_appserver}_pendingjobs.txt","w")
  puts "list of pending jobs stored in /root/smangam/#{$ncm_appserver}_pendingjobs.txt"
  cmd="ssh root@#{$ncm_appserver} ls /opt/smarts-ncm/data/appserver/pops"
  all_ds=`#{cmd}`.chomp.split
  all_ds.each { |x|
    cmd="ssh root@#{$ncm_appserver} ls -ltr /opt/smarts-ncm/data/appserver/pops/#{x}/syssync/commmgr/toServer |grep -v dsevents|grep -v total|grep -v status|wc -l"
    count=`#{cmd}`.chomp
    if count.to_i > 0
      pop=x[3..-1]
      cmd="cat /root/smangam/#{$ncm_appserver}_cflist.txt|grep '^POP'|grep #{pop}|awk '{print $3}'"
      cdm=`#{cmd}`.chomp
      f.puts "pending files for #{cdm} with pop# #{x}"
      cmd="ssh root@#{$ncm_appserver} ls -ltr /opt/smarts-ncm/data/appserver/pops/#{x}/syssync/commmgr/toServer |grep -v dsevents|grep -v total"
      f.puts `#{cmd}`
    end
  }
  f.close
...
def self.apg_backend_instances
  #get the backends
  cmd="ssh root@#{$apg_cdm_backend_host} ls /opt/APG/Backends/APG-Backend"
  output=`#{cmd}`.chomp
  backends = output.split(" ")

  backends.each do |backend|
    cmd="ssh root@#{$apg_cdm_backend_host} cat /opt/APG/Backends/APG-Backend/#{backend}/conf/socketinterface.xml|grep listen|cut -d'>' -f2|cut -d'<' -f1"
    myport=`#{cmd}`.chomp
    puts "Backend instance on #{$apg_cdm_backend_host}: #{backend}. The listening port is #{myport}"
  end
end

def self.apg_backend_mysql_info
  #get the backends
  cmd="ssh root@#{$apg_cdm_backend_host} ls /opt/APG/Backends/APG-Backend"
  output=`#{cmd}`.chomp
  backends = output.split(" ")
  ...
  backends.each do |backend|
    cmd="ssh root@#{$apg_cdm_backend_host} cat /opt/APG/Backends/APG-Backend/#{backend}/conf/mysql.xml|grep 'connection url'"
    mydata=`#{cmd}`.chomp
    puts "Backend instance on #{$apg_cdm_backend_host}: #{backend}. MySQL database destination is #{mydata}"
  end
end

def self.backend_tmpdir_status
  if $apg_cdm_backend_host == nil
    print "enter backend host:"
    $apg_cdm_backend_host=gets.chomp
  else
    puts "backend host is #{$apg_cdm_backend_host}"
  end
  #get the backends
  cmd="ssh root@#{$apg_cdm_backend_host} ls /opt/APG/Backends/APG-Backend"
  output=`#{cmd}`.chomp
  ...
  backends.each do |backend|
    puts "Backend instance on #{$apg_cdm_backend_host}: #{backend}"
    cmd="ssh root@#{$apg_cdm_backend_host} cat /opt/APG/Backends/APG-Backend/#{backend}/conf/config.xml|grep temporary-files|grep -v '<!--'|awk '{print $3}'|cut -f2 -d'>'|cut -f1 -d'<'"
    tmpdir=`#{cmd}`.chomp
    cmd="ssh root@#{$apg_cdm_backend_host} ls -ltr /opt/APG/Backends/APG-Backend/#{backend}/#{tmpdir} | grep -v total |wc -l 2>/dev/null"
...
def self.backend_socket_connections
  ...
  print "enter #{$apg_cdm_backend_host} backend listening port number:"
  myport=gets.chomp
  cmd="ssh root@#{$apg_cdm_backend_host} netstat -anp|grep '"+myport+" '"
  puts cmd
  puts `#{cmd}`
end
...
def self.apg_cdm_failover_filter_status_all
  ...
  printf("%20s :%-5s %-15s %-15s %-20s %-15s %-15s %-8s\n","cdm","count","/opt-freeMB","/-freeMB","backend_host","backend_port","dbhost","db")
  File.open("/root/smangam/apg_cdm_list.txt").each { |a|
    x=a.chomp.downcase
    if host_exists?(x)
      if $apg_cdm_failover_storage_dir!=nil
        ...
        count= `#{cmd}`.chomp
        if count.to_i >1
          printf("%20s :%-5s %-15s %-15s %-20s %-15s %-15s %-8s\n",x,count,$cdm_opt_freemb,$cdm_root_freemb,$apg_cdm_backend_host,$apg_cdm_backend_port,$apg_cdm_dbhost,$apg_cdm_db)
        end
      end
    end
    }
...
def self.apg_manage_qdf_files(cdm)
  ...
  cmd="ssh root@am-bch ls -ltr /opt/APG/Coll*/Fail*/Default/tmp-backend/*qdf|head -n 1|awk '{print $9}'"
end
