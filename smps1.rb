...
def sysmon_status(cdm,myresults,report_only)
...
def fs_opt_status(cdm,myresults,cdm_type,report_only)
  ...
  if !(output.empty?) then
    ...
    # delete samlog logs that are older than 27 days and larger than 1M
    if (cdm_type == "solo") then
     cmd = "ssh root@"+cdm +" find /opt/InCharge9/SAM/smarts/regional/logs -mtime +15 -size +1M"
    else
     cmd = "ssh root@"+cdm +" find /opt/InCharge9/SAM/smarts/customer/#{cdm}/logs -mtime +15 -size +1M 2>/dev/null"
    end
    output = `#{cmd}`
    if ($?.success?) then
      myresults.puts "#{mytime}: deleting samlog files older than 27 days for #{cdm} "
      output_arr = output.split(/\n/)
    ...
    # delete samlog archive logs that are older than 27 days and larger than 1M
    if (cdm_type == "solo") then
     cmd = "ssh root@"+cdm +" find /opt/InCharge9/SAM/smarts/regional/logs/archives -mtime +15 -size +1M"
    else
     cmd = "ssh root@"+cdm +" find /opt/InCharge9/SAM/smarts/customer/#{cdm}/logs/archives -mtime +15 -size +1M 2>/dev/null"
    end
    output = `#{cmd}`
    if ($?.success?) then
      myresults.puts "#{mytime}: deleting samlog archive files older than 27 days for #{cdm} "
      output_arr = output.split(/\n/)
      output_arr.each do |f|
        cmd = "ssh root@"+cdm +" rm -rf #{f}"
        `#{cmd}`
...
def program_status()
 cmd = "ps -eaf|grep smps.rb|grep -v grep |wc -l"
 output = `#{cmd}`.chomp
...
report_only = false
program_status()
myresults = File.new("/root/smangam/smps_output.txt","a")
mytime = Time.new.to_s.chomp
myresults.puts "\n***begin*** #{mytime}"
...
File.open("/root/smangam/cdm_solo.txt").each do |x|
 cdm = x.chomp
 cdm_type = "solo"
 apg_status(cdm,myresults)
 smarts_status(cdm,myresults)
 oiadapter_status(cdm,myresults,cdm_type)
 hyperic_status(cdm,myresults)
 vcmaster_status(cdm,myresults)
 sysmon_status(cdm,myresults,report_only)
 mem_status(cdm,myresults)
 fs_opt_status(cdm,myresults,cdm_type,report_only)
end
...
["amcdlncmapp01","amcdlncmapp02","amcdlncmapp03","amcdlncmapp04"].each do |cdm|
  cdm_type = "solo"
  vcmaster_status(cdm,myresults)
  fs_opt_status(cdm,myresults,cdm_type,report_only)
end
